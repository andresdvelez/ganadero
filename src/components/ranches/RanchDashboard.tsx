...
import { SmartRanchEditor } from "./SmartRanchEditor";
...
export function RanchDashboard({ ranchId }: RanchDashboardProps) {
  ...
  const updateRanchField = trpc.farm.updateField.useMutation({
    onSuccess: () => {
      getRanchQuery.refetch();
    }
  });

  const updateRanchBulk = trpc.farm.updateBulk.useMutation({
    onSuccess: () => {
      getRanchQuery.refetch();
    }
  });
  ...
  const handleFieldUpdate = async (field: string, value: any) => {
    await updateRanchField.mutateAsync({
      id: ranchId,
      field,
      value
    });
  ...
  const handleBulkUpdate = async (updates: Record<string, any>) => {
    await updateRanchBulk.mutateAsync({
      id: ranchId,
      updates
    });
  };
  ...
  return (
    <DashboardLayout>
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <Button
              ...
              onClick={() => {
                ...
                if (!isSmartMode) {
                  addToast({
                    variant: "success",
                    title: "Modo Inteligente Activado",
                    description: "Usa comandos de voz o IA para actualizar datos"
                  });
                }
              }}
            ...
            >
              ...
              {isSmartMode ? "Modo Inteligente" : "Activar IA"}
            </Button>
            <Button
              variant="outline"
              onClick={() => {
        ...
        {/* Smart Mode Conditional Rendering */}
        {isSmartMode ? (
          <SmartRanchEditor
            ranch={ranch}
